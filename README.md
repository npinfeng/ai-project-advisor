...
- 证据驱动：工具返回可读观察与结构化来源两部分；各页面、文件、Release、Issue 独立记录正文、URL、时间、版本、内容哈希及截断标记。
- 语义核验与定向补证：逐项判断硬约束，区分“内建”“需集成”“明确不支持”“尚未证实”和“来源冲突”；引用校验未通过则退回未知，最多补证一轮。
- 确定性评分：七个维度分别绑定原文引用，无有效依据的维度最高 5 分；加权分数不覆盖硬约束门禁，明确不支持者不可成为首选，未知或冲突者仅供有条件考察。
- 持久化 Hybrid RAG：Evidence 同步进入 Chroma 向量索引与 BM25；中文采用无需词典的短语/二元组分词，中英混合技术词同时保留完整词和子词。检索经过统一分数域的融合、保留原始查询的多查询召回和带稳定兜底的 Reranker；默认使用可配置的 BGE-M3 多语言 Embedding，并在模型配置变化时自动重建不兼容向量索引。
...
确定性任务展开与强类型路由
     ...
     └──► Documentation Researcher ──► Web + 只读 RAG + 文档 MCP 白名单
          │
          ▼
 Evidence 去重、持久化 → 无工具语义核验 → 引用与覆盖检查
     └──► 明确缺口时最多补充一次
          │
          ▼
      无工具 Reviewer（结构化输出）
...
- `primary` 表示工具取得的源文内容，不代表内容必然正确或属于官方权威来源；`search_snippet`、`discovery`、`inference`、`unverified` 不可直接支撑确定性需求结论或评分引用。
- GitHub 引用定位到具体文件、Release 或 Issue；批量抓取不把多页正文混绑到每个 URL。RAG 按原 Evidence ID 回取已有记录，不刷新采集时间、不改变原候选归属，也不将检索摘要当作新证据。
- 每轮研究后，核验节点使用 `research_model` 为有原文证据的候选逐一判断 `required_features`、部署和预算约束；自定义要求也会核验。语言与团队水平保留为偏好。该阶段受并发、超时和结构化重试配置限制，Token 计入运行统计；发生补证时会再次核验，因此增加模型调用开销。
- 程序检查候选归属、来源类型、逐字引用、语句边界和版本依据，并回填真实 URL 与字符偏移；证据缺失、引用无效或核验失败均保留为未知。语义是否真正蕴含结论仍由模型判断，引用匹配不等于事实准确率保证。
- 报告仅汇总通过引用校验的逐维理由，不直接发布模型无引用的自由分析；明确违反硬约束的候选即使加权分最高，也不会被选为首选。
- 旧知识库记录仍可读取，但缺少来源分类时默认 `unverified`。需要重新抓取原文才能用于新的确定结论；不会批量将旧摘要升级为原文证据。
...
当前完整回归结果为 `114 passed`。测试覆盖专业化工具隔离、角色 Skill 加载、仓库文件/目录读取、文档链接发现、Tool 参数校验/超时/重试/fan-out 限制、Agent Run 端到端超时、Reviewer 上下文预算、RAG 五组消融、Evidence 生命周期与版本冲突、Golden Case 独立审核门禁、真实发布预检与套件哈希绑定、异步 RAG、Reranker 并发闸门、日志上下文隔离、领域异常映射、确定性任务展开、硬约束定向补证、证据缺口收敛、单次补充研究门禁、手动候选结构化计划、健康状态降级、无模型报告渲染、Evidence 重启恢复、SQLite checkpoint/任务存储恢复、孤儿任务恢复、SSE 中断恢复、BM25/Chroma 持久化、SSE 真实 Evidence 采集和可信评测闭环。

新增来源与核验测试覆盖：多来源正文隔离、搜索摘要分类、GitHub 文件定位、RAG 原始证据复用、伪造引用/删减否定词/跨候选引用拒绝、版本冲突、逐维评分降级，以及硬约束核验→评分→报告链路。此组测试使用本地源文夹具和模拟模型响应，验证数据链路与门禁，不代表真实模型语义准确率；上线前仍需运行前述真实评测及独立人工审核。

RAG 检索基准采用 schema v3：支持 0–3 级 qrels、MAP@K、中文/业务类别切片、逐查询错误分析和数据集 SHA-256 身份。`--require-publishable-baseline` 会拒绝合成数据、Hash Embedding、未审核或非独立标注、缺少隐私/快照信息、查询不足及配置不可比的“基线”。完整流程见 [RAG 基准说明](docs/rag_benchmark.md)，数据结构可从 `evals/rag_quality_dataset.example.json` 复制。
